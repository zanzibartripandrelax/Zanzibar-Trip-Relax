...
import { 
  ...
  Ban, Upload, FileText, AlertCircle, CheckCircle2, Trash2, Calendar, Eye, Edit 
} from 'lucide-react';
...
export interface AdminDateRangeFilterConfig {
  key: string;
  label: string;
}
...
interface AdminDataTableProps<T> {
  ...
  dateRangeFilters?: AdminDateRangeFilterConfig[];
  csvFilename?: string;
  csvHeaders?: string[];
  ...
  // Explicit Row Actions requested
  onViewDetails?: (item: T) => void;
  onEdit?: (item: T) => void;
  onDelete?: (item: T) => void;
  viewDetailsLabel?: string;
  isEditDisabled?: boolean | ((item: T) => boolean);
  isDeleteDisabled?: boolean | ((item: T) => boolean);
}
...
export function AdminDataTable<T extends Record<string, any>>({
  ...
  statusOptions = [],
  dateRangeFilters = [],
  csvFilename = "admin_data.csv",
  csvHeaders,
  csvRowMapper,
  onExportSuccess,
  emptyMessage = "No administrative records found.",
  pageSize = 10,
  onBulkUpload,
  ...
  // Helper to resolve button state (disabled)
  const isActionDisabled = (disabledProp: boolean | ((item: T) => boolean) | undefined, item: T): boolean => {
    if (typeof disabledProp === 'function') {
      return disabledProp(item);
    }
    return !!disabledProp;
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col xl:flex-row gap-4 justify-between items-stretch xl:items-center bg-[#0A1224] border border-white/5 p-4 rounded-2xl shadow-sm">
        {/* Search Input */}
        {searchKeys.length > 0 && (
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full text-xs bg-[#121B30] border border-white/10 rounded-full py-2.5 pl-9 pr-4 text-white placeholder-slate-500 focus:outline-none focus:border-[#D4A017] transition-all"
      ...
      {dateRangeFilters && dateRangeFilters.length > 0 && showDateFilter && (
        <div className="bg-[#0A1224] border border-[#D4A017]/15 p-4 rounded-2xl space-y-3.5 shadow-lg relative overflow-hidden transition-all duration-300">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pl-3">
            ...
            {(startDate || endDate) && (
              <button
                ...
                className="text-[10px] text-red-400 hover:text-red-350 font-bold flex items-center gap-1 transition-colors cursor-pointer border border-red-500/10 px-2 py-1 bg-red-500/5 rounded-lg"
              >
                <Trash2 size={11} />
      ...
      <div className="bg-[#0A1224] border border-white/5 rounded-2xl overflow-hidden shadow-sm">
        {loading ? (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-[#121B30] text-slate-300 font-bold border-b border-white/5">
                  ...
                  {/* Action Column Header if callbacks are supplied */}
              ...
              <tbody className="divide-y divide-white/5">
                {currentPagedData.map((item, rowIdx) => (
                  ...
                  >
                    ...
                    {/* Integrated dynamic Row Actions */}
                    {(onViewDetails || onEdit || onDelete) && (
                        ...
                        <div className="flex gap-1.5 justify-end items-center">
                          {onViewDetails && (
                            <button
                              ...
                              className="bg-[#121B30] hover:bg-[#1f2a46] px-2.5 py-1.5 rounded-lg text-[10px] font-bold text-slate-300 transition-colors cursor-pointer border border-white/5 flex items-center gap-1"
                            >
                              <Eye size={12} />
                          ...
                          {onEdit && (
                            <button
                              ...
                              disabled={isActionDisabled(isEditDisabled, item)}
                              className="bg-amber-500/10 hover:bg-amber-500/20 disabled:opacity-30 disabled:cursor-not-allowed px-2.5 py-1.5 rounded-lg text-[10px] font-bold text-amber-400 transition-colors cursor-pointer border border-amber-500/15 flex items-center gap-1"
                              title={isActionDisabled(isEditDisabled, item) ? "Requires elevated permission level to edit" : "Edit Details"}
                            >
                              <Edit size={12} />
                              <span>Edit</span>
                            </button>
                          ...
                          {onDelete && (
                            <button
                              ...
                              disabled={isActionDisabled(isDeleteDisabled, item)}
                              className="bg-red-500/10 hover:bg-red-500/20 disabled:opacity-30 disabled:cursor-not-allowed px-2.5 py-1.5 rounded-lg text-[10px] font-bold text-red-400 transition-colors cursor-pointer border border-red-500/15 flex items-center gap-1"
                              title={isActionDisabled(isDeleteDisabled, item) ? "Requires elevated permission level to delete" : "Delete record"}
                            >
                              <Trash2 size={12} />
                              <span>Delete</span>
                            </button>
          ...
          <div className="py-20 text-center text-slate-400 space-y-3">
            <Ban className="w-10 h-10 mx-auto text-slate-500" />
