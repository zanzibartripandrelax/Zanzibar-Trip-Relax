...
const DESTINATIONS = [
  { id: 'tours', label: 'Zanzibar Tours', desc: 'Stone Town, marine safaris & spice farms' },
  { id: 'safaris', label: 'Tanzania Safaris', desc: 'Serengeti, Ngorongoro & mainland wildlife' },
  { id: 'packages', label: 'Holiday Packages', desc: 'All-inclusive multi-day beach vacations' },
  { id: 'kilimanjaro', label: 'Kilimanjaro Treks', desc: 'Machame, Lemosho & classic routes' },
  { id: 'transfers', label: 'Airport Transfers', desc: 'Private luxury hotel pickups & taxis' }
];
...
export default function SmartSearchEngine({ navigate, initiallyOpenCalendar = false }: SmartSearchEngineProps) {
  ...
  const handleSearchSubmit = () => {
    ...
    localStorage.setItem('ztr_search_destination_id', selectedDest.id);
    localStorage.setItem('ztr_search_destination_label', selectedDest.label);
    ...
    navigate(selectedDest.id as Page);
  };
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-2 relative" id="quick-search">
      {/* Floating Search Container */}
      <div className="bg-white rounded-2xl md:rounded-full shadow-2xl p-4 md:p-2 border border-slate-100 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 md:gap-1 max-w-5xl mx-auto bg-opacity-98 backdrop-blur-md">
        
        {/* Destination Picker */}
        <div ref={destRef} className="flex-1 relative min-w-[200px]">
          <button
            type="button"
            onClick={() => {
            ...
            className="w-full text-left px-5 py-3 md:py-2.5 rounded-xl md:rounded-full hover:bg-slate-50 transition-colors flex items-center justify-between gap-2 cursor-pointer group"
          >
            <div className="flex items-center gap-3">
              <div className="p-2 bg-amber-50 rounded-lg text-[#0B3B8C] shrink-0 group-hover:scale-105 transition-all">
                <MapPin size={18} />
              </div>
              <div className="truncate">
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Where are you going?</span>
                <span className="block text-sm font-bold text-[#0A1224] truncate">{selectedDest.label}</span>
              </div>
            </div>
          ...
          <AnimatePresence>
            {showDestDropdown && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="absolute left-0 right-0 md:left-2 md:right-auto md:w-[320px] top-full mt-2 bg-white rounded-2xl border border-slate-100 shadow-2xl p-3 z-50 overflow-hidden"
              >
                <div className="space-y-1">
                  <span className="block text-[10px] font-extrabold text-slate-450 uppercase tracking-widest px-3 py-1 bg-slate-50 rounded-lg mb-2">Experience Category</span>
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-between group cursor-pointer ${
                        selectedDest.id === dest.id 
                          ...
                          : 'text-[#0A1224] hover:bg-slate-50'
                      }`}
                    >
                      <div>
                        <span className="block font-bold">{dest.label}</span>
                        <span className={`block text-[10px] font-medium mt-0.5 ${
                          selectedDest.id === dest.id ? 'text-white/80' : 'text-slate-400'
                        }`}>{dest.desc}</span>
                      </div>
                    </button>
                  ))}
        ...
        {/* Date Picker Button */}
        <div ref={dateRef} className="flex-1 relative min-w-[220px]">
          <button
            type="button"
            onClick={() => {
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Travel Dates</span>
                <span className="block text-sm font-bold text-[#0A1224] truncate max-w-[190px]">
                  {dateRangeLabel}
                </span>
              </div>
            </div>
          ...
          <AnimatePresence>
            {showCalendar && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="absolute left-1/2 -translate-x-1/2 md:left-auto md:right-auto md:translate-x-0 md:-left-20 top-full mt-2 bg-white rounded-3xl border border-slate-100 shadow-2xl p-6 z-50 w-[95vw] md:w-[680px] overflow-hidden"
              >
                {/* Calendar Header */}
        ...
        {/* Guests Picker */}
        <div ref={guestRef} className="flex-1 relative min-w-[180px]">
          <button
            type="button"
            onClick={() => {
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="truncate">
                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Guests</span>
                <span className="block text-sm font-bold text-[#0A1224] truncate max-w-[150px]">{guestsLabel}</span>
              </div>
            </div>
            <ChevronDown size={14} className="text-slate-400 shrink-0" />
          </button>
          ...
          <AnimatePresence>
            {showGuestsDropdown && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="absolute right-0 top-full mt-2 bg-white rounded-2xl border border-slate-100 shadow-2xl p-4 z-50 w-full sm:w-[280px] space-y-4"
              >
                {/* Adults Count */}
                ...
                <button
                  ...
                  className="w-full bg-[#0B3B8C] text-white font-extrabold text-xs uppercase tracking-wider py-2.5 rounded-full hover:bg-opacity-95 transition-all cursor-pointer"
                >
                  Apply Guests
                </button>
              </motion.div>
        ...
        <button
          ...
          className="bg-[#D4A017] hover:bg-[#c49010] text-[#0A1224] font-black rounded-xl md:rounded-full px-6 py-4 md:py-3.5 transition-all flex items-center justify-center gap-2 shadow-lg hover:scale-[1.02] active:scale-[0.98] duration-200 cursor-pointer text-xs uppercase tracking-wider md:h-12 border-0"
        >
